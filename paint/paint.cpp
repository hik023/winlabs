...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   HWND hWnd = CreateWindowW(L"Balaev", L"Paint", WS_OVERLAPPEDWINDOW,
...
LRESULT CALLBACK MyWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		...
		{
			hpen_new = CreatePen(0, 4, RGB(rand() % 255, rand() % 255, rand() % 255));
			SelectObject(hdc, hpen_new);
			Ellipse(hdc, LOWORD(lParam) - 2, HIWORD(lParam) - 2, LOWORD(lParam) + 2, HIWORD(lParam) + 2);
			DeleteObject(hpen_new);
		}
		else if(wParam == MK_SHIFT) {
			//hrgndo = CreateEllipticRgn(LOWORD(lParam) - 13, HIWORD(lParam) - 13, LOWORD(lParam) + 13, HIWORD(lParam) + 13);
			//FillRgn(hdc, hrgndo, (HBRUSH)(CreateSolidBrush(RGB(0, 0, 0))));
			hpen_new = CreatePen(0, 4, RGB(255,255,255));
			SelectObject(hdc, hpen_new);
			Ellipse(hdc, LOWORD(lParam) - 10, HIWORD(lParam) - 10, LOWORD(lParam) + 10, HIWORD(lParam) + 10);
			//FillRgn(hdc, hrgndo, (HBRUSH)(CreateSolidBrush(RGB(255, 255, 255))));
			//DeleteObject(hrgndo);
			DeleteObject(hpen_new);
		}
